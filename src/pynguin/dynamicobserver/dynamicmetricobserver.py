...
class DynamicMetricObserver(so.SearchObserver):
    ...
    def _init_calculators(self) -> None:
        ...
        self._calculators.append(calc.DiversityCalculatorExperimental())
...
class GenerationObserver(so.SearchObserver):
    ...
    def before_first_search_iteration(self, initial: tsc.TestSuiteChromosome) -> None:
        ...
        self._collect_test_cases(initial)

    def after_search_iteration(self, best: tsc.TestSuiteChromosome) -> None:
        self.iterations += 1
        self.fitness_values.append(self._collect_fitness_values(best))
        self._collect_test_cases(best)

    def after_search_finish(self) -> None:
        end_time = time.time_ns()
        self._logger.info(f"Search Duration: {end_time - self.start_time} ns")
        self._writer.write_fitness_values(self.fitness_values)
    ...
    def _collect_fitness_values(self, generation: tsc.TestSuiteChromosome) -> FitnessValues:
        ...
        return FitnessValues(self.iterations, fitness_values, generation.get_coverage())

    def _collect_test_cases(self, generation: tsc.TestSuiteChromosome) -> None:
        export_visitor = export.PyTestChromosomeToAstVisitor()
        test_cases: list[str]= []
        for testcase in generation.test_case_chromosomes:
            testcase.accept(export_visitor)
            test_cases.append(self.helper.module_to_string(export_visitor.to_module(), format_with_black=config.configuration.test_case_output.format_with_black))
        self._writer.write_test_cases(test_cases, self.iterations)
